...
pnl_history, leftovers into unmatched_sells, and open lots into pnl_lots.
...
def test_multi_lot_fifo_persists_one_fill_and_open_lot(engine):
    ...
    assert len(hist) == 1
    assert hist[0]["realized_pnl"] == pytest.approx(25.0)
    assert hist[0]["cost_basis"] == pytest.approx(20.0)
    assert hist[0]["proceeds"] == pytest.approx(45.0)

    lots = db.get_pnl_lots(WALLET, CONTRACT)
    assert len(lots) == 1
    assert lots[0]["amount_remaining"] == pytest.approx(5.0)
...
def test_oversell_persists_unmatched_remainder(engine):
    ...
    assert db.get_pnl_history(WALLET)[0]["realized_pnl"] == pytest.approx(5.0)
...
def test_wash_trade_records_nothing(engine):
    ...
    lots = db.get_pnl_lots(WALLET, CONTRACT)
    assert lots[0]["amount_remaining"] == pytest.approx(10.0)
